...
ZIP_NAME="WytaoFBrowser-${VERSION}-macos-${ARCH}.zip"
APP_EXPORT="$OUTPUT_DIR/WytaoFBrowser-${VERSION}-macos-${ARCH}.app"
STAGE_DIR="$STAGING_ROOT/$TARGET"
APP_STAGE="$STAGE_DIR/WytaoF Browser.app"
...
echo "========================================"
echo "  WytaoF Browser macOS Publish"
echo "========================================"
echo "Target : $TARGET"
...
if [[ "$SKIP_BUILD" -ne 1 ]]; then
  ...
  (
    cd "$ROOT_DIR"
    wails build -s -platform "darwin/$ARCH" -o wytaof-browser
  )
else
